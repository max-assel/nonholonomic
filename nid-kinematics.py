...
class KinematicsController(object):
    ...
    def initialize_xi(self):
        self.xi = np.empty(3)
        self.xi[0] = 2             # x-position
        self.xi[1] = 3             # y-position
        self.xi[2] = np.pi * 7 / 4 # theta
        return xi

    def dynamics(self, u):
        f = np.empty(3)

        f[0] = cos(self.xi[2]) * u[0] # x-velocity
        f[1] = sin(self.xi[2]) * u[0] # y-velocity
...
def update(iter):
    ...
    xi = update_state(lambda t, xi: state_equation(xi, tau), (t, t + self.dt), xi)
    lambda_ = update_lambda(
        lambda t, l_: calc_lambda_dot(l_, clambda, epsilon), (t, t + self.dt), lambda_
    )
